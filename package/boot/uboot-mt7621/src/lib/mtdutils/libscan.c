...
int fs_nand_erase_block_scan_progress = 0;
...
int ubi_scan(struct mtd_info *mtd, struct ubi_scan_info **info, int verbose)
{
	...
	si->ec = calloc(mtd_eraseblock_count(mtd), sizeof(uint32_t));
	if (!si->ec) {
	...
	verbose(v, "start scanning eraseblocks 0-%d", mtd_eraseblock_count(mtd));
	for (eb = 0; eb < mtd_eraseblock_count(mtd); eb++) {
		...
		tlt_leds_check_anim();
		fs_nand_erase_block_scan_progress = (long long)(eb + 1) * 100 / mtd_eraseblock_count(mtd);

		if (v) {
			normsg_cont("scanning eraseblock %d", eb);
		}
		if (pr) {
			printf("\r" PROGRAM_NAME ": scanning eraseblock %d -- %2d %% complete", eb,
			       fs_nand_erase_block_scan_progress);
		}
		...
		ret = mtd_read(mtd, eb * mtd->erasesize, sizeof(ech), &retlen, (u_char*) &ech);
		if (ret < 0)
			goto out_ec;
	...
	si->good_cnt = mtd_eraseblock_count(mtd) - si->bad_cnt;
