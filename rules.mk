...
device_shortname = $(call qstrip,$(subst DEVICE_TEMPLATE_teltonika_,,$(subst DEVICE_teltonika_,,$(subst _test,,$(or $(CONFIG_TARGET_MULTI_PROFILE_NAME),$(CONFIG_TARGET_PROFILE))))))
...
REBUILT_PKG_FILE:=$(TMP_DIR)/.pkg-rebuilt

define pkg_rebuilt_info
  [ -w "$(REBUILT_PKG_FILE)" ] && echo "$(1)" >> "$(REBUILT_PKG_FILE)"|| true
endef
...
INSTALL_BIN_USR:=install -m0750
INSTALL_SUID:=install -m4755
INSTALL_DIR:=install -d -m0755
INSTALL_DATA:=install -m0644
INSTALL_CONF:=install -m0600
INSTALL_CONF_USR:=install -m0660
