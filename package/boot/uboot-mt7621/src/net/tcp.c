...
#define DEF_PENDING_CLOSE_MS 5000
...
void receive_tcp(struct ip_hdr *ip, int len, struct ethernet_hdr *et)
{
	...
	if (flags & TCP_RST) {
		if (c->status != SYN_RCVD) {
			...
			assert((size_t) c->cb > CFG_SYS_SDRAM_BASE);
			c->cb(&cbd);
		}
...
static void tcp_conn_check(struct tcp_conn *c)
{
	...
	if (c->close_time && now >= c->close_time) {
		c->close_time = 0;
		cbd.status = TCP_CB_CLOSED;
		assert((size_t) c->cb > CFG_SYS_SDRAM_BASE);
		c->cb(&cbd);
...
int fs_finished_responding_to_all_requests = 0;
...
void tcp_periodic_check(void)
{
	...
	if (num == 0)
		fs_finished_responding_to_all_requests = 1;
	else
		fs_finished_responding_to_all_requests = 0;
	if (tcp_stop && !num)
...
int tcp_set_pending_close_conn(const void *conn, u32 time_ms)
{
	struct tcp_conn *c = (struct tcp_conn *) conn;
	...
	if (!time_ms) {
		time_ms = DEF_PENDING_CLOSE_MS;
	}

	u32 now = get_timer(0);
	c->close_time = now + time_ms;
