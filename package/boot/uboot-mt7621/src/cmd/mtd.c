			...
			char *const argv[])
{
	...
	while (remaining) {
		...
		remaining -= mtd->erasesize;
		fs_nor_erasing_progress = (len - remaining) * 100 / len;
		if (fs_allow_external_tcp_loop_execution)
			execute_tcp_net_loop_once();
		erase_op.addr += mtd->erasesize;

		if (fs_nor_erasing_progress != last_printed_progress_percentage) {
			printf("\rmtd: Erasing %d %% complete", fs_nor_erasing_progress);
			last_printed_progress_percentage = fs_nor_erasing_progress;
		}
	}
	printf("\n");
