			   ...
			   struct tcb_cb_data *cbd)
{
	...
	*p = 0;
	...
	if (!tlt_leds_get_flashing_state())
		printf("%s %s\n", pdata->buf, uri_ptr);
	...
	if (method == HTTP_POST) {
		...
		if (hdr_size + pdata->payload_size < sizeof(pdata->buf)) {
			...
			pdata->upload_ptr =
				(char *) CFG_SYS_SDRAM_BASE + 0x10000;
				 ...
				 struct tcb_cb_data *cbd)
{
	...
	if (req->method == HTTP_SERIAL) {
		mnf_get_field("sn", pdata->buf);
		printf("Sending serial %s\n", pdata->buf);
		pdata->request.urih = &dummy_urih;
		pdata->response.status = HTTP_RESP_STD;
		pdata->resp_std_cnt = 1;
		/* send response */
		tcp_send_data(cbd->conn, pdata->buf,strlen(pdata->buf));
...
static void httpd_tx(struct httpd_instance *inst, struct tcb_cb_data *cbd)
{
	...
	if (pdata->response.info.connection_pending_close)
		tcp_set_pending_close_conn(cbd->conn, 5000);
