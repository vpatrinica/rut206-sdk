...
int fs_mmc_erasing_progress	  = 0;
int fs_mmc_image_writing_progress = 0;
...
#endif
{
	...
	while (blk < blkcnt) {
		...
		fs_mmc_erasing_progress = blk * 100 / blkcnt;
		if (fs_allow_external_tcp_loop_execution){
			execute_tcp_net_loop_once(); // To bring back the progress to the client
			tlt_leds_check_anim();
		}
...
#endif
{
	...
	do {
		...
		fs_mmc_image_writing_progress = (blkcnt - blocks_todo) * 100 / blkcnt;
		if (fs_allow_external_tcp_loop_execution){
			execute_tcp_net_loop_once(); // To bring back the progress to the client
			tlt_leds_check_anim();
		}
