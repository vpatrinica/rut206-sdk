...
static void eth_mt7628_write_common_regs(void __iomem *sw_base)
{
	writel(0xc8a07850, sw_base + MT7628_SWITCH_FCT0);
	writel(0x00000000, sw_base + MT7628_SWITCH_SGC2);
	writel(0x00405555, sw_base + MT7628_SWITCH_PFC1);
	writel(0x00007f7f, sw_base + MT7628_SWITCH_POC0);
	writel(0x00007f7f, sw_base + MT7628_SWITCH_POC2);
	writel(0x0002500c, sw_base + MT7628_SWITCH_FCT2);
	writel(0x0008a301, sw_base + MT7628_SWITCH_SGC);
	writel(0x02404040, sw_base + MT7628_SWITCH_SOCPC);
	writel(0x3f502b28, sw_base + MT7628_SWITCH_FPA1);
	writel(0x00000000, sw_base + MT7628_SWITCH_FPA);
	writel(0x7d000000, sw_base + MT7628_SWITCH_BMU_CTRL);
}

static void eth_mt7628_map_ports(ofnode eth_node, bool disable_all)
{
	void __iomem *sw_base;
	phys_addr_t sw_phys;
	u32 val;

	sw_phys = ofnode_get_addr_index(eth_node, 1);
	if (sw_phys == FDT_ADDR_T_NONE) {
		printf("MT7628 switch registers not found\n");
		return;
	}

	sw_base = map_physmem(sw_phys, 0x8000, MAP_NOCACHE);
	if (!sw_base) {
		printf("Failed to map MT7628 switch registers\n");
		return;
	}

	eth_mt7628_write_common_regs(sw_base);

	if (disable_all) {
		writel(0x00000000, sw_base + MT7628_SWITCH_PVIDC0);
		writel(0x00000000, sw_base + MT7628_SWITCH_PVIDC0 + 0x4);
		writel(0x00000000, sw_base + MT7628_SWITCH_PVIDC0 + 0x8);
		writel(0x00000000, sw_base + MT7628_SWITCH_PVIDC0 + 0xc);
		writel(0x00000000, sw_base + MT7628_SWITCH_VMSC0);
	} else {
		val = 0x1001;
		writel(val, sw_base + MT7628_SWITCH_PVIDC0);
		writel(val, sw_base + MT7628_SWITCH_PVIDC0 + 0x4);
		writel(val, sw_base + MT7628_SWITCH_PVIDC0 + 0x8);
		writel(val, sw_base + MT7628_SWITCH_PVIDC0 + 0xc);
		writel(0xffff407f, sw_base + MT7628_SWITCH_VMSC0);
	}

	unmap_physmem(sw_base, MAP_NOCACHE);
}

static void eth_mt7628_disable_all_ports(ofnode eth_node)
{
	eth_mt7628_map_ports(eth_node, true);
}

static void eth_mt7628_restore_normal_ports(ofnode eth_node)
{
	eth_mt7628_map_ports(eth_node, false);
}

static ofnode eth_mt7628_get_node(void)
{
	ofnode eth_node;

	eth_node = ofnode_by_compatible(ofnode_null(), "mediatek,mt7628-eth");
	if (!ofnode_valid(eth_node))
		eth_node = ofnode_path("/eth@10110000");
...
int eth_init(void)
{
	...
	ofnode mt7628_eth_node;
	int ret = -ENODEV;

	mt7628_eth_node = eth_mt7628_get_node();
	if (ofnode_valid(mt7628_eth_node))
		eth_mt7628_restore_normal_ports(mt7628_eth_node);
...
static bool eth_initalized = false;
#endif

int eth_lazy_init(void) {
#ifdef CONFIG_NET_LAZY
	if (!eth_initalized) {
		puts("Net:   ");
		eth_initialize();
...
int eth_initialize_port_separation(void)
{
	ofnode eth_node = ofnode_null();
	struct reset_ctl rst;
	int ret;

	eth_common_init();

	eth_node = ofnode_by_compatible(ofnode_null(), "mediatek,mt7621-eth");
	if (!ofnode_valid(eth_node))
		eth_node = ofnode_path("/eth@1e100000");
	if (ofnode_valid(eth_node)) {
		ret = reset_get_by_index_nodev(eth_node, 2, &rst);
		if (ret) {
			printf("Failed to get reset 2: %d\n", ret);
			return -EINVAL;
		}
	...
	if (ofnode_valid(eth_node))
		eth_mt7628_disable_all_ports(eth_node);
