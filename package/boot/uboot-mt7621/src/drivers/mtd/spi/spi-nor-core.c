...
int fs_nor_image_writing_progress = 0;
	...
	size_t *retlen, const u_char *buf)
{
	...
	ssize_t ret;
	int last_printed_progress_percentage = -1;
	...
	for (i = 0; i < len; ) {
		...
		*retlen += written;
		...
		if (fs_allow_external_tcp_loop_execution) { // Only when flashing FW via httpd
			fs_nor_image_writing_progress = i * 100 / len;
			execute_tcp_net_loop_once();
			tlt_leds_check_anim(); 
			if (fs_nor_image_writing_progress != last_printed_progress_percentage) {
				printf("\rspi_nor: Writing %d %% complete", fs_nor_image_writing_progress);
				last_printed_progress_percentage = fs_nor_image_writing_progress;
			}
		}
	}
...
int spi_nor_scan(struct spi_nor *nor)
{
	...
	printf("\nSF: Detected %s with page size ", nor->name);
