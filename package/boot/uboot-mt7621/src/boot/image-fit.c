...
/**
 * fit_get_desc - get node full_hwver property
 * @fit: pointer to the FIT format image header
 * @noffset: node offset
 * @desc: double pointer to the char, will hold pointer to the description
 *
 * fit_get_full_hwver() reads HW version property from a given node, if
 * description is found pointer to it is returned in third call argument.
 *
 * returns:
 *     0, on success
 *     -1, on failure
 */
int fit_get_full_hwver(const void *fit, int noffset, char **hwver)
{
	int len;

	*hwver = (char *)fdt_getprop(fit, noffset, FIT_FULL_HWVER_PROP, &len);
	if (*hwver == NULL) {
		fit_get_debug(fit, noffset, FIT_FULL_HWVER_PROP, len);
		return -1;
	}
