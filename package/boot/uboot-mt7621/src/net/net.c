...
static void cleanup_and_bootp(void)
{
	net_cleanup_loop();
	eth_halt();
...
/*
 *	One iteration of the for loop inside of net_loop()
 *  Used to handle http requests while executing flashing process
 */

extern int fs_finished_responding_to_all_requests;
extern int fs_allow_external_tcp_loop_execution;
extern int fs_start_fw_flashing;

void execute_tcp_net_loop_once(void){
	eth_rx();
	tcp_periodic_check();
	return;
}
...
int net_loop(enum proto_t protocol)
{
	...
	int httpd_timeout_enabled = 0, httpd_wait_msec = 0;
	char *env_timeout_enabled;
...
restart:
	...
	env_timeout_enabled = env_get("httpd_timeout_enabled");
	if (env_timeout_enabled && strcmp(env_timeout_enabled, "1") == 0) {
		httpd_timeout_enabled = 1;
	}

	for (;;) {
		if (httpd_timeout_enabled) {
			udelay(1000 * HTTPD_TIMEOUT_DELAY_MSEC);
		...
		if (fs_finished_responding_to_all_requests && fs_start_fw_flashing &&
		    fs_allow_external_tcp_loop_execution) // Good to start FW flashing process
			try_to_flash_new_fw_image();

		/*
		 *	Abort if ctrl-c was pressed.
		 */
		if (ctrlc() || (httpd_timeout_enabled && httpd_wait_msec >= HTTPD_TIMEOUT_MSEC)) {
